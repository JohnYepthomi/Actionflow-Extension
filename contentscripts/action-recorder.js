...
class ActionsRecorder {
  constructor() {
    ...
    this.SUPPORTED_EVENTS = ["mouseup", "input"];
  ...
  async clickHandler(e) {
    ...
    switch (NODE_TYPE) {
      case "Select":
        ...
        const temp_value = select_el.value;
        const availableOptions = Array.from(select_el.options).map((op_el) => {
          if (op_el.value === "") {
        ...
        const selectedOption = availableOptions.find(
          (e) => e.toUpperCase() === temp_value.toUpperCase()
        );
        const selectProps = {
  ...
  switch (request.message) {
    ...
    case "element-pick":
      if (
        ...
        return;
      if (this.isActive) this.tempInactive = true;
...
(async () => {
  ...
  if (window.location.href.includes("http://localhost:3000/?state=")) {
    console.log("Getting Access Token from Parent site");
